...
-- Global variables untuk kontrol animasi paksa
_G.AnimBlockConnection = nil
_G.AnimMonitorConnection = nil
_G.ToolConnections = {Backpack = nil, Character = nil}

-- [[ DATA TABLES ]] --
local rockCategories = {
    ["Ghost Rock"] = {
        {"Ghost Jungle", 10000000}, {"Ghost Muscle King", 5000000}, {"Ghost Legend", 1000000},
        {"Ghost Eternal", 750000}, {"Ghost Mythical", 400000}, {"Ghost Frozen", 150000},
        {"Ghost Golden", 5000}, {"Ghost Starter", 100}, {"Ghost Tiny", 0}
    },
    ["Teleport Rock"] = {
    ...
    ["Emote Rock"] = {
        {"Emote Jungle", 10000000}, {"Emote Muscle King", 5000000}, {"Emote Legend", 1000000},
        {"Emote Eternal", 750000}, {"Emote Mythical", 400000}, {"Emote Frozen", 150000},
        {"Emote Golden", 5000}, {"Emote Starter", 100}, {"Emote Tiny", 0}
    }
}
...
-- LOGIKA BLOKIR PAKSA (Sesuai kode request kamu)
local function ForceBlockAnims(state)
    if state then
        local function checkAndStop(track)
            local anim = track.Animation
            local name = track.Name:lower()
            -- Jangan hentikan jika itu adalah emote yang dipilih
            if anim and selectedEmoteName and tostring(anim.AnimationId):find(tostring(emoteList[selectedEmoteName])) then
                return
            end
        ...
        if humanoid then
            for _, track in pairs(humanoid:GetPlayingAnimationTracks()) do checkAndStop(track) end
            _G.AnimBlockConnection = humanoid.AnimationPlayed:Connect(checkAndStop)
        end

        _G.AnimMonitorConnection = game:GetService("RunService").Heartbeat:Connect(function()
            local h = player.Character and player.Character:FindFirstChild("Humanoid")
            if h then
                for _, track in pairs(h:GetPlayingAnimationTracks()) do checkAndStop(track) end
            end
        end)
    else
        -- Clean up / Munculkan kembali (Disconnect semua)
        ...
        pcall(function() 
            ...
            if animator then
                for _, t in pairs(animator:GetPlayingAnimationTracks()) do
                    if tostring(t.Animation.AnimationId):find(tostring(emoteList[selectedEmoteName])) then
...
local ButtonSection = farmTab:Section({Title = "Rocks Ultimate Boost", Icon = "list"})

farmTab:Dropdown({
    Title = "Change Rock Mode",
    Desc = "To Change Which Rock You Want",
...
local ButtonSection = farmTab:Section({Title = "Start Punch Rock", Icon = "mountain"})
local RockToggles = {}
...
for i = 1, 9 do
    ...
    RockToggles[i] = farmTab:Toggle({
        Title = "!!! Select Rock !!!",
        Desc = "Rock Ultimate Mode",
        Callback = function(state)
            -- Validasi Emote
            if state and currentCategory == "Emote Rock" and not selectedEmoteName then
                WindUI:Notify({Title = "Select Your Emote First", Duration = 5})
            ...
            if state then
                task.spawn(function()
                    ...
                    if currentCategory == "Emote Rock" then
                        ForceBlockAnims(true) -- Aktifkan blokir paksa
                        playEmote()
                        task.wait(0.5)
                    end
                    ...
                    while toggleStates[i] do
                        ...
                        if data and hrp and dur and dur.Value >= data[2] then
                            ...
                            if targetRock then
                                local hitParts = {"LeftHand", "RightHand"}
                ...
                -- Toggle OFF: Matikan blokir animasi & reset punch
                ForceBlockAnims(false) 
...
function RefreshRockButtons()
    local locations = rockCategories[currentCategory]
    for i = 1, 9 do
        if locations[i] then RockToggles[i]:SetTitle(locations[i][1]) end
    end
end
RefreshRockButtons()
