...
farmPlusTab:Input({
    ...
    Callback = function(text) 
        local function parseFormat(val)
            val = string.lower(string.gsub(val, "%s+", ""))
            
            if string.sub(val, -2) == "qa" then
                return (tonumber(string.sub(val, 1, -3)) or 0) * 10^15
            elseif string.sub(val, -2) == "qi" then
                return (tonumber(string.sub(val, 1, -3)) or 0) * 10^18
            end
            
            local suffix = string.sub(val, -1)
            if suffix == "k" then
                return (tonumber(string.sub(val, 1, -2)) or 0) * 10^3
            elseif suffix == "m" then
                return (tonumber(string.sub(val, 1, -2)) or 0) * 10^6
            elseif suffix == "b" then
                return (tonumber(string.sub(val, 1, -2)) or 0) * 10^9
            elseif suffix == "t" then
                return (tonumber(string.sub(val, 1, -2)) or 0) * 10^12
            else
                return tonumber(val) or 0
            end
        end
        TargetStrength = parseFormat(text)
    end
})

farmPlusTab:Toggle({
    Title = "Auto Rep TarGet",
    Desc = "Will Stop When Strength Reaches Target (using tools first)",
...
local playerDropdown = Crystal:Dropdown({
    Title = "Select Player",
...
Crystal:Button({
    ...
    Icon = "refresh-cw",
    Callback = function()
        playerDropdown:Refresh(buildPlayerDisplayList(), true)
    end
})
